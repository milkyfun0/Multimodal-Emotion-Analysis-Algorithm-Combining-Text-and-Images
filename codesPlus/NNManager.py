...
from codesPlus.ImageRegionNet import ImageRegionNet
...
class Net(nn.Module):
    ...
    def __init__(self, nHidden, seqLen, dropout=0, numLayers=1, classEmbeddingPath="..//ExtractWords/vector",
        ...
        self.device = device
        self.imageRegionNet = ImageRegionNet(device=device)
    ...
    def forward(self, X):
        ...
        with torch.no_grad():
            self.imageRegionNet.eval()
            images = self.imageRegionNet(images).detach()
