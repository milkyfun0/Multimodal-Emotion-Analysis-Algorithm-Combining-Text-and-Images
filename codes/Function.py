...
def generateData(sqLen=80, batchSize=64, alpha=0.7, rate=0.0):
    if input("是否重新生成数据集(Y/N):") != "Y":
        return
    dataAugment = DataAugment(seqLen=sqLen, batchSize=batchSize, imgDir=readImagesDir,
                              textPath=textPrefix + "textQ.txt",
    ...
    dataList = dataAugment.start(rate=rate, imgSaveDir=imagePrefix + "/augmentImages/", image2ClassSaveDir=classPrefix,
                                 alpha=alpha)
    with open(textPrefix + "/textQ.txt", "r", encoding="utf-8") as file:
...
def preMain(sqLen=75, batchSize=64, alpha=0.7, rate=0.0):
    """
    :param sqLen:
    :param batchSize:
    ...
    :return:
    """
    generateData(sqLen=sqLen, batchSize=batchSize, alpha=alpha, rate=rate)
